...
mkdir -p "$content"
# maps.txt is <name> <category> <source> <url>; category and source drive mapcycle
# generation, not fetching.
while read -r name category source url; do
  case "$name" in ''|\#*) continue ;; esac
  [ -n "${url:-}" ] || { echo "malformed line for ${name}: expected <name> <category> <source> <url>" >&2; exit 1; }
  echo "  fetching $name"
